...
<p>The output of <code>constrained_birkhoff_von_neumann_decomposition(X,constraint_structure)</code> is a list. The first entry is the distribution over basis matrices, the second entry is the list of basis matrices, the third and fourth entries are checks that the distribution probabilities sum to one and that the average of the basis matricies is the target matrix <code>X</code>.</p>
