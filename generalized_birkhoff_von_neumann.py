...
def generalized_birkhoff_von_neumann_iterator(H):
  ...
  eligible_edges = [(from_node,to_node,edge_attributes) for from_node,to_node,edge_attributes in G.edges(data=True) if all(i < edge_attributes['weight'] or edge_attributes['weight'] < i for i in range(0,math.floor(sum(sum(X) ) )+1 ) ) ]
...
def iterator_of_generalized_birkhoff_von_neumann_iterator(G):
  H=[(G,1)]
  solution=[]
  while len(H) > 0:
